...
for f=1:length(files)
    ...
    %% # MB added
    if ismember(sessionID{s},monashSessionID)
        allfiles = dir([datafolder,subject_folder{s}, filesep, '*C*.vhdr']);
        loadName = allfiles(f).name;        
        EEG0 = pop_loadbv([datafolder,subject_folder{s}, filesep], loadName);
        EEG.data = [];
        EEG.data = double(EEG0.data);
        EEG.times = [];
        EEG.times = EEG0.times;
        EEG.pnts = [];
        EEG.pnts = EEG0.pnts;
    end
    %%
    %# MB changed. fs is already 500hz for both datasets
    if EEG.srate~=fs
    ...
    EEG = pop_importeyetracker(EEG,ET_matfiles{f},[first_event last_event]...
        ,[1:4] ,{'TIME' 'GAZE_X' 'GAZE_Y' 'AREA'},0,1,0,0);
    ...
    %% Downsample to 500Hz %% MB = commented
    %     resamp_data = [];
    %     EEG.data = double(EEG.data);
    ...
    for i=1:numev
        if ~startsWith(EEG2.event(i).type,'S','IgnoreCase',true) && ~startsWith(EEG2.event(i).type,'condition','IgnoreCase',true) && isnan(str2double(EEG2.event(i).type))
            EEG2.event(i).type = [];
            EEG2.event(i).type = 0;
        else
...
save([pathOut, subject_folder{s} '/' subject_folder{s} '_',num2str(bandlimits(1,1)),'_to_',num2str(bandlimits(1,2)), ...
